...
//			out.println("Code for " + frame.getLabel());
    ...
    private List<IRStm> toList(IRStm s) {
        List<IRStm> ans = empty();
        while (s instanceof SEQ) {
            SEQ ss = (SEQ) s;
            ans = cons(ss.left, ans);
            s = ss.right;
        }
        ans = cons(s, ans);
        return ans.reverse();
    ...
    public void bodyIsTS() {
        traceScheduled = toList(body);
    }
